...
# ── Helper: persiste áudio no R2 para sobreviver a redeploys do Render ────────
def _upload_audio_to_r2(local_path: str, job_id: str) -> str | None:
    """
    Faz upload do áudio original para R2 e retorna a URL pública.
    Usado imediatamente após receber o upload do usuário para garantir
    que o arquivo não seja perdido em redeploys (o /tmp é efêmero no Render).
    Retorna None silenciosamente se R2 não estiver configurado.
    """
    try:
        from config import get_r2_client, R2_BUCKET_NAME, R2_PUBLIC_URL
        r2 = get_r2_client()
        if not r2:
            return None
        ext = os.path.splitext(local_path)[1].lower() or ".mp3"
        key = f"audio/{job_id}/original{ext}"
        with open(local_path, "rb") as f:
            r2.put_object(Bucket=R2_BUCKET_NAME, Key=key, Body=f,
                          ContentType="audio/mpeg" if ext == ".mp3" else "audio/wav")
        url = f"{R2_PUBLIC_URL.rstrip('/')}/{key}" if R2_PUBLIC_URL else None
        if url:
            print(f"   ✅ Áudio salvo no R2: {url}")
        return url
    except Exception as e:
        print(f"   ⚠️ Upload áudio R2 falhou (não crítico): {e}")
        return None


# ── Helper: garante que o áudio existe localmente (baixa do R2 se necessário) ─
def _ensure_audio_local(job: dict) -> str | None:
    """
    Retorna o caminho local do áudio do job.
    Se o arquivo sumiu do /tmp (redeploy do Render), baixa do R2 automaticamente.
    Retorna None se não for possível recuperar.
    """
    audio_path = job.get("audio_path")
    if audio_path and os.path.exists(audio_path):
        return audio_path

    # Tenta recuperar do R2
    audio_r2_url = job.get("audio_r2_url")
    if not audio_r2_url:
        print("   ❌ audio_r2_url não salvo — áudio irrecuperável")
        return None

    try:
        import requests as _req
        ext = os.path.splitext(audio_r2_url)[1].lower() or ".mp3"
        recovered_path = os.path.join(UPLOAD_DIR, f"{job['id']}_recovered{ext}")
        print(f"   ⬇️  Recuperando áudio do R2: {audio_r2_url[:70]}")
        r = _req.get(audio_r2_url, timeout=120, stream=True)
        if r.status_code != 200:
            print(f"   ❌ Download R2 falhou: HTTP {r.status_code}")
            return None
        with open(recovered_path, "wb") as f:
            for chunk in r.iter_content(chunk_size=8192):
                f.write(chunk)
        print(f"   ✅ Áudio recuperado: {recovered_path}")
        # Atualiza o caminho no job em memória
        job["audio_path"] = recovered_path
        return recovered_path
    except Exception as e:
        print(f"   ❌ Erro ao recuperar áudio do R2: {e}")
        return None
...
async def generate_video(
    ...
    # ✅ Persiste áudio no R2 imediatamente — protege contra redeploy do Render
    audio_r2_url = _upload_audio_to_r2(audio_path, job_id)
...
async def regen_lipsync_clip(
    ...
    # Verifica se áudio ainda existe — recupera do R2 se necessário
    audio_path = _ensure_audio_local(jobs_db[job_id])
    if not audio_path:
        raise HTTPException(
            400,
            "Áudio não encontrado no servidor. "
            "Abra o painel de Lip Sync e reenvie o arquivo de áudio."
        )
...
async def generate_lipsync_video(
    ...
    if audio and audio.filename:
        ...
        with open(audio_path, "wb") as f:
            f.write(await audio.read())
        # ✅ Persiste no R2 para não perder em redeploy
        new_r2_url = _upload_audio_to_r2(audio_path, job_id)
        jobs_db[job_id]["audio_path"]    = audio_path
        jobs_db[job_id]["audio_r2_url"]  = new_r2_url or jobs_db[job_id].get("audio_r2_url")
        jobs_db[job_id]["vocals_path"]   = None
        update_job(job_id, audio_path=audio_path, vocals_path=None,
                   audio_r2_url=jobs_db[job_id]["audio_r2_url"])
    else:
        audio_path = _ensure_audio_local(jobs_db[job_id])
        if not audio_path:
            raise HTTPException(400, "Audio do job nao encontrado no servidor. Reenvie o arquivo de áudio.")
...
def process_merge(job_id: str):
    ...
    try:
        ...
        # ✅ Recupera áudio do R2 se /tmp foi limpo pelo Render
        audio_path = _ensure_audio_local(job)
        if not audio_path:
            print("   ⚠️ Áudio não encontrado nem no R2 — merge será feito sem áudio")
...
def _preextract_vocals(job_id: str):
    try:
        ...
        # ✅ Recupera áudio do R2 se local foi perdido
        audio_path = _ensure_audio_local(job)
        if not audio_path:
            print("   ⚠️ Áudio não disponível para extração de vocals")
            return
